...
function generateEnhancedSummary(section, outline, partIndex = null, totalParts = null) {
  ...
  if ((!section.heading && section.level === 0) || (!section.headings && !section.heading)) {
    // 获取文档标题（如果存在）
    const docTitle = outline.length > 0 && outline[0].level === 1 ? outline[0].title : 'Document';
    return `${docTitle} Preface`;
  }
  ...
  if (section.headings && section.headings.length > 0) {
    ...
    for (const heading of sortedHeadings) {
      ...
      const headingIndex = outline.findIndex(item => 
        item.title === heading.heading && item.level === heading.level
      );
    ...
    // 如果没有有效的标题，返回默认摘要
    if (paths.length === 0) {
      return section.heading ? section.heading : 'Unnamed Paragraph';
    }
  ...
  // 查找当前段落在大纲中的位置
  const currentHeadingIndex = outline.findIndex(item => 
    item.title === section.heading && item.level === section.level
  );
